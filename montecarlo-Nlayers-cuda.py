...
class DipoleSim:
    ...
    def __init__(self, a: float, c1: float, c2: float, layers: int, rows: int, columns: int, temp0,
        ...
        self.c_sqs_even = cp.zeros((layers, 1))
        self.c_sqs_odd = cp.zeros((layers, 1))
        self.layer_distances = cp.zeros((layers, layers, 1))
    ...
    def calc_energy(self):
        ...
        px = cp.ravel(self.p[:, :, 0])
        py = cp.ravel(self.p[:, :, 1])

        rx = cp.tile(self.r[:, 0], (self.layers, 1))
        ry = cp.tile(self.r[:, 1], (self.layers, 1))
        rz = cp.zeros((self.layers, self.N))
        ...
        for jj in range(self.N_total-1):
            ...
            term1 = cp.sum(pi_dot_pj / r_sq ** 1.5)
            term2 = cp.sum(pi_dot_dr * pj_dot_dr / r_sq ** 2.5)

            energy += self.k_units * term1 - 3. * term2
        energy += cp.sum(self.E[0] * px) + cp.sum(self.E[1] * py)
        return energy
    ...
    def calc_energy2(self):
        ...
        """
        # arrange all the x- and y-values in 1 x l*N arrays
        px = cp.array([cp.ravel(self.p[:, :, 0])])  # 1 x 2N
        py = cp.array([cp.ravel(self.p[:, :, 1])])

        # duplicate xy values of r into 1 x l*N arrays
        rx = cp.zeros((1, self.N_total))
        ry = cp.zeros((1, self.N_total))
        ...
        for ll in range(1, self.layers):
            ...
            c1s = int(cp.ceil(layer_diff_half))
        ...
        r_sq[r_sq == 0] = cp.inf  # this removes self energy

        p_dot_r_sq = (px.T * dx + py.T * dy) * (px * dx + py * dy)
        energy_ext_neg = cp.sum(self.E * self.p)
        energy_int = cp.sum(p_dot_p / r_sq ** 1.5)
        energy_int -= cp.sum(3 * p_dot_r_sq / r_sq ** 2.5)
        # need to divide by 2 to avoid double counting
        return 0.5 * self.k_units * cp.sum(energy_int) - energy_ext_neg

    def calc_polarization(self):
        """
        :return: net dipole moment per unit volume
        """
        return cp.sqrt(cp.sum(cp.sum(self.p, axis=(0, 1)) ** 2)) / self.volume
    ...
    def step(self):
        ...
        if dp[0] and dp[1]:
            ...
            r_sq = cp.tile(cp.sum(dr * dr, axis=1), (self.layers, 1))
            r_sq += self.layer_distances[trial_layer]

            r_sq[r_sq == 0] = cp.inf  # remove self energy
            p_dot_dp = cp.sum(self.p * dp, axis=2)  # array: 2 x N
            r_dot_p = cp.sum(self.p * dr, axis=2)  # array: 2 x N
            r_dot_dp = cp.sum(dr * dp, axis=1)  # array: N
            # energy_decrease is positive if the energy goes down and negative if it goes up
            energy_decrease = cp.sum((r_dot_dp * r_dot_p) * 3. / r_sq ** 2.5 - p_dot_dp / r_sq ** 1.5) * self.k_units
            energy_decrease += cp.sum(self.E * dp)
    ...
    def change_electric_field(self, efield_x: float, efield_y: float):
        ...
        """
        old_E = self.E
        self.E = cp.array([efield_x, efield_y])
        neg_delta_energy = cp.sum((old_E - self.E) * self.p)
    ...
    @staticmethod
    def gen_dipoles_triangular(a: float, rows: int, columns: int) -> cp.ndarray:
        """
        ...
        x = 0.5 * a
        y = a * np.sqrt(3) * 0.5
        r = cp.zeros((rows * columns, 2))
        for jj in range(rows):
            start = jj * rows
            r[start:start + columns, 0] = cp.arange(columns) * a + x * jj
            r[start:start + columns, 1] = cp.ones(columns) * y * jj
        return r

    @staticmethod
    def gen_dipoles_triangular2(a: float, rows: int, columns: int) -> cp.ndarray:
        """
        ...
        x = 0.5 * a
        y = a * np.sqrt(3) * 0.5
        r1 = cp.arange(columns) * a
        r1 = cp.tile(r1, (rows, 1))
        r1[1::2] += x
        r1 = cp.ravel(r1)
        r2 = (cp.ones((rows, 1)) * cp.arange(columns)) * y
        r2 = cp.ravel(r2.T)
        return cp.column_stack((r1, r2))

    @staticmethod
    def gen_dipoles_square(a: float, rows: int, columns: int) -> cp.ndarray:
        """
        ...
        r1 = cp.tile(r1, (rows, 1))
        r1 = cp.ravel(r1)
        r2 = (cp.ones((rows, 1)) * cp.arange(columns)) * a
        r2 = cp.ravel(r2.T)
        return cp.column_stack((r1, r2))

    def gen_dipole_orientations(self) -> cp.ndarray:
        """
        Initialize dipole directions
        :return: 2 x N x 2 array
        """
        p_directions = cp.zeros((self.layers, self.N, 2))  # 1st is number of layers, 2nd dipoles, 3rd vector size
    ...
    def create_ori_vec(orientations_num: int) -> np.ndarray:
        ...
        orientations = cp.zeros((orientations_num, 2))
        for e in range(orientations_num):
            orientations[e] = cp.array([np.cos(del_theta * e), np.sin(del_theta * e)])
        return orientations
...
def cool_down(layers, rows, columns, temperatures, smoothing=None):
    ...
    for p1, c1, oddness1 in zip(range(2), (1.78, 1.27), ("even", "odd")):
        for p2, c2, oddness2 in zip(range(2), (1., 1.52), ("even", "odd")):
            energies = cp.zeros(len(temperatures))
            p_temperature = cp.zeros((len(temperatures), layers, 2))
            p_total = cp.zeros(len(temperatures))
            if smoothing:
                energies_std = cp.zeros(len(temperatures))
            ...
            for tt, temperature in enumerate(temperatures):
                ...
                if smoothing is None:
                    for _ in range(5):
                        sim.run_over_system()
                    p_temperature[tt] = cp.sum(sim.p, axis=1)
                    energies[tt] = sim.energy
                    # sim.save_img(t_string)
                else:
                    ps_to_smooth = cp.zeros((smoothing, layers, 2))
                    energies_to_smooth = cp.zeros(smoothing)
                    for ss in range(smoothing):
                        for _ in range(20):
                            sim.run_over_system()
                        ps_to_smooth[ss] = cp.sum(sim.p, axis=1)
                    ...
                    energies[tt] = cp.sum(energies_to_smooth) / smoothing
                    energies_std[tt] = cp.sqrt(cp.sum((energies_to_smooth - energies[tt]) ** 2) / smoothing)
                    p_temperature[tt] = cp.sum(ps_to_smooth, axis=0) / smoothing
                    p_to_ave = cp.sum(cp.sum(ps_to_smooth, axis=1) ** 2, axis=1)
                    p_total[tt] = cp.sum(p_to_ave) / smoothing

                    p_std[tt] = cp.sqrt(cp.sum((p_temperature[tt] - p_total[tt]) ** 2) / smoothing)
            plt.figure()
            if smoothing is None:
                p_total = cp.sqrt(cp.sum(cp.sum(p_temperature, axis=1) ** 2), axis=1)
