...
class DipoleSim:
    ...
    def __init__(self, a: float, c: float, rows: int, columns: int, layers: int, temp0, dipole_strength: float,
    ...
    def rotate_dipole(self, layer: int, row: int, column: int):
        """
        Rotate dipole in layer at row, column and accept if favorable (Metropolis)
        :param layer: index
        :param row: index
        :param column: index
        :return:
        """
        dipole_index = self.columns * row + column
        trial_p = self.orientations[self.rng.integers(self.orientations_num)]  # array: 2
        # bitwise AND with 1 returns True for odd and False for even numbers
        if layer & 1 and self.odd:
            trial_p = -trial_p
        dp = trial_p - self.p[layer, dipole_index, :]
        if dp[0] and dp[1]:
            r_sq = np.zeros((2, self.N))  # array: 2 x N
            dr = self.r - self.r[dipole_index]  # array: N x 2
            r_sq[dipole_index, :] = np.sum(dr * dr, axis=1)  # array: N (same layer)
            r_sq[(dipole_index + 1) % 1, :] = r_sq[dipole_index, :] + self.c_sq  # array: N (other layer)
            ...
            if random.random() < np.exp(self.beta * energy_decrease):
                self.accepted += 1
                self.p[dipole_index, dipole_index, :] = trial_p
...
if __name__ == "__main__":
    ...
    sim = DipoleSim(a=1.1, c=0.1, rows=30, columns=30,
